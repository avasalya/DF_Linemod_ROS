...
if __name__ == '__main__':
    ...
    n = 16
    files = []
    randomList = rand.sample(range(1, n), 1)
    scrpath = ['/media/ash/SSD/Odaiba/dataset/onigiri-real/']

    for i in randomList:
        files.append(scrpath[0] + str(i))
        # files.append(scrpath[0] + str(4))

    pe = pose_estimation(mask_rcnn, pose, refiner, objId, scaled, files)
    viz = pe.pose()
    # plt.imsave('seg_result/out/pose' + str(i) +  '.png', viz)
